...
- [ ] **Step 1: Author `src/taskpane/skills/powerpoint/slides.md`** — must cover:
  ...
  - **Adding slides:** `presentation.slides.add({ slideMasterId?, layoutId? })` is the typed path (PowerPointApi 1.3). When neither is provided, the new slide uses the default master and its first layout. When only `layoutId` is provided, that layout must be available under the default master. The method returns `void` — to operate on the new slide, re-query (e.g. `slides.getItemAt(items.length)` after sync).
